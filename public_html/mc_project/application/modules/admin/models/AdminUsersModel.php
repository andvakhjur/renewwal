...
class Admin_Model_AdminUsersModel extends Mc_Model
{
    /**
    ...
    {
        $this->_table = new Admin_Model_DbTable_AdminUsersDbTable();
    ...
    {
        return new Admin_Form_AdminUsersForm($options);
    }

    public function getFormFilter($options = null)
    {
        return new Admin_Form_Filter_AdminUsersFilter($options);
    }
    ...
    public function getArticle($id, $toArray = true)
    {
        $return = parent::getArticle($id, $toArray);
        //$return['permissions'] = 
        ...
        if(!empty($return)) {
            $return['permissions'] = $table_permissions->getAdapter()->fetchPairs($select);
        }
