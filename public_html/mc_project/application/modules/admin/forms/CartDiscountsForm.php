...
class Admin_Form_CartDiscountsForm extends Mc_Form {
    ...
    public function init() {
        ...
        $groupName = 'group_' . $groupNumb++;
        //--------------------------------------------------------------------//
        $elementName = 'summ_bottom';
        $this->groups[$groupName][$elementName] = new Zend_Form_Element_Text($elementName);
        $this->groups[$groupName][$elementName]->setLabel('Нижняя сумма: ');
        $this->groups[$groupName][$elementName]->setAttrib('size', 50);
        $this->groups[$groupName][$elementName]->addValidator(new Zend_Validate_Float(array(
            "locale" => "en_US",
        )));
        //--------------------------------------------------------------------//
        $elementName = 'summ_top';
        $this->groups[$groupName][$elementName] = new Zend_Form_Element_Text($elementName);
        $this->groups[$groupName][$elementName]->setLabel('Верхняя сумма: ');
        $this->groups[$groupName][$elementName]->setAttrib('size', 50);
        $this->groups[$groupName][$elementName]->addValidator(new Zend_Validate_Float(array(
            "locale" => "en_US",
        )));
        ...
        $this->groups[$groupName][$elementName] = new Zend_Form_Element_Text($elementName);
        $this->groups[$groupName][$elementName]->setLabel('Скидка (%): ');
        $this->groups[$groupName][$elementName]->setAttrib('size', 50);
        $this->groups[$groupName][$elementName]->addValidator(new Zend_Validate_Float(array(
            "locale" => "en_US",
        )));
...
//            $catalogSections = new Admin_Model_CartDiscountsModel();
