...
class Admin_Form_Filter_AdminResourcesFilter extends Mc_Form_Filter
{
    public $groups = array();
    ...
    public function init()
    {
        ...
        $this->groups[$groupName][$elementName]->addDecorator('Labelsid', array('before' => '<dt>Меню-родитель:</dt><dd>'));
        ...
        if (count($resourcesArr) > 0) {
            foreach ($resourcesArr as $resourcesArr_key => $resourcesArr_item) {
                ...
                while($par_id) {
                    $flag = false;
                    foreach ($resourcesArr as $key => $item) {
                        if($item['id'] == $par_id) {
                            $par_id = $item['par_id'];
                ...
                $options[$resourcesArr_item['id']] =  $parsTitle . $resourcesArr_item['title'];
        ...
        $this->groups[$groupName][$elementName] = new Zend_Form_Element_Select($elementName);
        $this->groups[$groupName][$elementName]->setAttrib('id', 'filter_' . $this->groups[$groupName][$elementName]->getName());
        $this->groups[$groupName][$elementName]->addDecorator('Labelsid', array('before' => '<dt>Показывать в меню:</dt><dd>'));
        //$this->groups[$groupName][$elementName]->setRequired(true);
        //$this->groups[$groupName][$elementName]->setAttrib('size', 40);
        ...
        $this->groups[$groupName][$elementName] = new Zend_Form_Element_Select($elementName);
        $this->groups[$groupName][$elementName]->setAttrib('id', 'filter_' . $this->groups[$groupName][$elementName]->getName());
        $this->groups[$groupName][$elementName]->addDecorator('Labelsid', array('before' => '<dt>Показывать в правах доступа:</dt><dd>'));
        //$this->groups[$groupName][$elementName]->setRequired(true);
        //$this->groups[$groupName][$elementName]->setAttrib('size', 40);
        $this->groups[$groupName][$elementName]->addMultiOptions(array(
            '' => 'все',
