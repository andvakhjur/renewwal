...
class Admin_ArticlesController extends Mc_Controller_Action
{
    ...
    public function init()
    {
        ...
        $this->_articlesModel = new Admin_Model_ArticlesModel();
        ...
        $this->_breadcrumbs->addCrumb(Admin_Model_ArticlesModel::getCrumb());
    }
    ...
    public function deleteAction()
    {
        ...
        $this->_helper->flashMessenger->addMessage(Zend_Registry::get('view')->translate('Данные успешно удалены'));
    ...
    public function addAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            if (sizeof($this->_getParam('cancel')) == '0') {
                if ($form->isValid($this->_getAllParams())) {// save the content item
                    ...
                    $this->_helper->flashMessenger->addMessage(Zend_Registry::get('view')->translate('Данные успешно добавлены'));
                    return $this->_helper->redirector('index');
                }
            } else {
                $this->_helper->flashMessenger->addMessage(Zend_Registry::get('view')->translate('Изменения отменены'));
                return $this->_helper->redirector('index');
            }
        }
        ...
        $this->_breadcrumbs->addCrumb(array(
            'label' => Zend_Registry::get('view')->translate('Добавление'),
        ));
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (sizeof($this->_getParam('cancel')) == '0') {
                if ($form->isValid($this->_getAllParams())) {// save the content item
                    ...
                    $this->_helper->flashMessenger->addMessage(Zend_Registry::get('view')->translate('Изменения успешно сохранены'));
                    return $this->_helper->redirector('index');
                }
            } else {
                $this->_helper->flashMessenger->addMessage(Zend_Registry::get('view')->translate('Изменения отменены'));
