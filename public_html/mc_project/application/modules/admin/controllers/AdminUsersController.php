...
class Admin_AdminUsersController extends Mc_Controller_Action
{
    ...
    public function init()
    {
        ...
        $this->_articlesModel = new Admin_Model_AdminUsersModel();
        ...
        $this->_breadcrumbs->addCrumb(Admin_Model_AdminUsersModel::getCrumb());
    }
        ...
        if ($this->getRequest()->isPost()) {
            if (sizeof($this->_getParam('cancel')) == '0') {
                if ($form->isValid($this->_getAllParams())) {// save the content item
                    $formed = $form->mapForm();
                    $permissions = $formed['permissions'];
                    unset($formed['permissions']);
                    $form->updateUserPermissions($this->_articlesModel->addArticle($formed), $permissions);
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (sizeof($this->_getParam('cancel')) == '0') {
                if ($form->isValid($this->_getAllParams())) {// save the content item
                    ...
                    $permissions = $formed['permissions'];
                    unset($formed['permissions']);
                    $form->updateUserPermissions($this->_articlesModel->editArticle($formed, $id), $permissions);
