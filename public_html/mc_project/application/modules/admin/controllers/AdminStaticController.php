...
class Admin_AdminStaticController extends Mc_Controller_Action
{
    ...
    public function init()
    {
        ...
        $this->_articlesModel = new Admin_Model_AdminStaticModel();
        ...
        if('yes' == $this->_getParam('admin')) {
            $this->_itsAdmin = true;
            $this->view->itsAdmin = $this->_itsAdmin;
        }
        ...
        if(true == $this->_itsAdmin) {
            $this->_breadcrumbs->addCrumb(Admin_Model_AdminStaticModel::getCrumb());
        }
    }
    ...
    {
        $this->_articlesModel->delete($this->_getParam('id'));
    ...
    public function delfieldsAction()
    {
        if ($this->getRequest()->isPost()) {
            if (sizeof($this->_getParam('field')) > 0) {
                $this->_articlesModel->delfields($this->_getParam('field'));
                $this->_helper->flashMessenger->addMessage('Данные успешно удалены');
            }
        }
        return $this->_helper->redirector('index');
    ...
    public function editAction()
    {
        ...
        switch($article['url']) {
            case 'static-social-link1':
            case 'static-social-link2':
            case 'static-social-link3':
                $form = new Admin_Form_AdminStaticSimpleForm();
                break;
            case 'services-price':
                $form = new Admin_Form_AdminStaticFileForm();
                break;
            case 'system-logo':
                $form = new Admin_Form_AdminStaticLogoForm();
                break;
            default:
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (sizeof($this->_getParam('cancel')) == '0') {
                if ($form->isValid($this->_getAllParams())) {// save the content item
                    ...
                    if('yes' != $this->_getParam('admin')) {
                        return $this->_redirector->gotoRoute(array('action' => 'edit'), 'admin-' . $article['url']);
                    } else {
                        return $this->_helper->redirector('index');
                    }
                }
