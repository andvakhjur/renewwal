...
class Admin_AdminResourcesController extends Mc_Controller_Action
{
    ...
    public function init()
    {
        ...
        $this->_articlesModel = new Admin_Model_AdminResourcesModel();
    ...
    public function indexAction()
    {
        ...
        if(empty($this->_sessionNamespace->filterValues->par_id)) {
            $this->_filterSql->where("`par_id` = '0' OR `par_id` = '' OR `par_id` IS NULL");
        }
        ...
        $breadcrumbs = array();
        $crumbsParId = $this->_sessionNamespace->filterValues->par_id;
        while($crumbsParId) {
            $crumbArticle = $this->_articlesModel->getArticle($crumbsParId);
            $breadcrumbs[] = array(
                'label' => $crumbArticle['title'],
                'uri' => $this->view->url(array('module' => Zend_Registry::get('view')->moduleName, 'controller' => 'admin-resources', 'action' => 'filter-by-par-id', 'value' => $crumbArticle['id']), 'default', true),
            );
            $crumbsParId = $crumbArticle['par_id'];
        }
        $breadcrumbs = array_reverse($breadcrumbs);
        
        $this->_breadcrumbs->addCrumb(Admin_Model_AdminResourcesModel::getCrumb());
        if (count($breadcrumbs) > 0) {
            foreach ($breadcrumbs as $breadcrumbs_key => $breadcrumbs_item) {
                $this->_breadcrumbs->addCrumb(array(
                    'label' => $breadcrumbs_item['label'],
                    'uri' => $breadcrumbs_item['uri'],
                ));
            }
        }
    ...
    public function addAction()
    {
        ...
        // крошки
        $breadcrumbs = array();
        $crumbsParId = $this->_sessionNamespace->par_id;
        while($crumbsParId) {
            $crumbArticle = $this->_articlesModel->getArticle($crumbsParId);
            $breadcrumbs[] = array(
                'label' => $crumbArticle['title'],
                'uri' => $this->view->url(array('module' => Zend_Registry::get('view')->moduleName, 'controller' => 'admin-resources', 'action' => 'show', 'par_id' => $crumbArticle['id']), 'default', true),
            );
            $crumbsParId = $crumbArticle['par_id'];
        }
        $breadcrumbs = array_reverse($breadcrumbs);
