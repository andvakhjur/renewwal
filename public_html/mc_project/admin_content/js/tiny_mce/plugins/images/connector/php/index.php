<?php
include 'config.php';
/*$f = fopen("/home/cis.lan/public_html/1.txt", "a+");
fputs($f, session_id()."-".$_POST['SID']."\n");
fclose($f);*/
					...
					if (preg_match('/[a-z0-9-_]+/sim', $_POST['name'])) {
						if(is_dir($dir.'/'.$_POST['name'])) {
							...
							if(mkdir($dir.'/'.$_POST['name'])) {
								if (copy($dir.'/.htaccess', $dir.'/'.$_POST['name'].'/.htaccess')) {
									...
									$result['error'] = 'Ошибка записи .htaccess';
								}
							} else {
...
//var_dump($this->ShowDir($path, $_POST['pathtype']));exit();
	...
	function UploadFile($dir, $type) {
		...
		if(!is_dir($dir.'/.thumbs')) {
			mkdir($dir.'/.thumbs');
			copy($dir.'/.htaccess', $dir.'/.thumbs/.htaccess');
		}
		...
		foreach ($dir as $v) {
            ...
            //if($v['general']['width'] > WIDTH_TO_LINK && $v['general']['height'] > HEIGHT_TO_LINK) {
				...
				if($v['general']['width'] > $v['general']['height']) {
					...
					$t = new Image_Toolbox(realpath(DIR_ROOT.'/'.$v['general']['path']).'/'.$v['general']['filename']);
					$t->newOutputSize(WIDTH_TO_LINK, 0, 0, false, '#FFFFFF');
					$t->save(realpath(DIR_ROOT.'/'.$v['general']['path']).'/'.$v['general']['filename'], 'jpg', 80);//*/
				} else {
...
//                        exit();
//                    }
                    $t->newOutputSize(0, HEIGHT_TO_LINK, 0, false, '#FFFFFF');
                    //$t->newOutputSize(0, 50, 0, false, '#FFFFFF');
